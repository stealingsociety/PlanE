...
from preprocess import data_process_classical
...
class Data(tgdata.Data):
    ...
    def __inc__(self, key, value, *args, **kwargs):
        ...
        match key:
            ...
            case "spqr_batch":
                return 1
            case "spqr_edge_index":
                return num_spqr_nodes
            # case 'spqr_read_from_g':    return [[num_g_nodes], [num_spqr_nodes], [0], [0]]
            case "spqr_read_from_e":
                return [
                    [num_spqr_nodes],
                    [num_g_nodes],
                    [num_g_nodes],
            ...
            case "b_batch":
                return 1
            case "b_read_from_spqr":
                return [[num_spqr_nodes], [num_b_nodes]]
            case "b_read_from_spqr_root":
                return [[num_spqr_nodes], [num_b_nodes]]

            case "g_read_from_b":
                return [[num_b_nodes], [num_g_nodes]]
            case "g_read_from_spqr":
                return [[num_spqr_nodes], [num_g_nodes]]

            case "bc_edge_index":
                return [[num_b_nodes], [num_g_nodes]]
            case "cb_edge_index":
                return [[num_g_nodes], [num_b_nodes]]
...
def process(data: tgdata.Data, directional_tree=True):
    if data.edge_attr is None:
        discrete_edge_attr = torch.zeros(
            (data.num_edges,), dtype=torch.long
        ).tolist()
    else:
        discrete_edge_attr = torch.unique(
            data.edge_attr, sorted=True, return_inverse=True
        )[1].tolist()

    if data.x is None:
        discrete_node_feature = torch.zeros(
            (data.num_nodes,), dtype=torch.long
        ).tolist()
    else:
        discrete_node_feature = torch.unique(
            data.x, sorted=True, return_inverse=True
        )[1].tolist()
    ...
    def add_cc(g: sageall.Graph):
        ...
        tree, bcc_tree_and_code = data_process_classical.encode_cc(
            g, discrete_node_feature, discrete_edge_feature
        )

        center = tree.center()[0]
        node_depth = {
            n: len(path) for n, path in tree.shortest_paths(center).items()
        }
        ...
        for node_tuple in tree.vertex_iterator():
            match node_tuple[0]:
                case "B":
                    ...
                    (spqr_tree, spqr_center, bcc_code) = bcc_tree_and_code[
                        node_tuple[1]
                    ]
                    ...
                    b_read_from_spqr_root.append(
                        [b_node_map[spqr_center], b_id]
                    )
                ...
                case "C":
                    ...
                    c_order[node_tuple[1]] = (
                        max_depth - node_depth[node_tuple]
                    ) // 2
    ...
    def add_bcc(tree: sageall.Graph, center, code):
        ...
        max_depth = max(node_depth.values())
        spqr_order.extend(
            [max_depth - node_depth[i] for i in sorted(node_depth.keys())]
        )
        cycle = code.get_cycles()
        for u, v in reversed(
            list(tree.breadth_first_search(center, edges=True))
        ):
    ...
    return Data(
        ...
        spqr_edge_index=torch.tensor(spqr_edge_index, dtype=torch.long)
        .view(-1, 2)
        .mT,
        spqr_edge_attr=torch.tensor(spqr_edge_attr, dtype=torch.long),
        spqr_read_from_e=torch.tensor(spqr_read_from_e, dtype=torch.long)
        .view(-1, 6)
        .mT,
        b_batch=torch.zeros(b_num_nodes, dtype=torch.long),
        b_read_from_spqr=torch.tensor(b_read_from_spqr, dtype=torch.long)
        .view(-1, 2)
        .mT,
        b_read_from_spqr_root=torch.tensor(
            b_read_from_spqr_root, dtype=torch.long
        )
        .view(-1, 2)
        .mT,
        g_read_from_b=torch.tensor(g_read_from_b, dtype=torch.long)
        .view(-1, 2)
        .mT,
        g_read_from_spqr=torch.tensor(g_read_from_spqr, dtype=torch.long)
        .view(-1, 2)
        .mT,
        b_order=torch.tensor(b_order, dtype=torch.long),
        c_order=torch.tensor(c_order, dtype=torch.long),
        bc_edge_index=torch.tensor(bc_edge_index, dtype=torch.long)
        .view(-1, 2)
        .mT,
