    ...
    if factor == -1:
        ...
        return nn.Sequential(
            ...
            (
                nn.Identity()
                if norm == "None"
                else nn.BatchNorm1d(dim_in * factor)
            ),
            nn.ReLU(),
            nn.Dropout(drop),
...
def SANDWICH(dim, p):
    return nn.Sequential(nn.BatchNorm1d(dim), nn.ReLU(), nn.Dropout(p))
...
class Embedding(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        match input_dim:
            case "ogb_atom_node":
                self.embed = AtomEncoder(output_dim)
            case "ogb_atom_edge":
                self.embed = BondEncoder(output_dim)
            case "lin":
                self.embed = nn.LazyLinear(output_dim)
...
class GINEMLPConv(tgnn.MessagePassing):
    def __init__(
        self,
        pre_nn: torch.nn.Module,
        post_nn: torch.nn.Module,
        eps: float = 0.0,
    ...
    def message(self, x_j, edge_attr):
        return self.pre_nn(torch.cat([x_j, edge_attr], dim=1))
