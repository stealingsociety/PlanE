...
def get_dataset(
    name: str, fn_final_transform=None, split: str = "", tag: str = "v0"
):
    ...
    match name:
        case "QM9" | "QM9NoE":
            ...
            fn_dataset = lambda split: QM9Dataset(
                root=root, split=split, pre_transform=pre_transform
            )
        case "QM9CC":
            from datasets.qm9 import QM9Dataset
            ...
            fn_dataset = lambda split: QM9Dataset(
                root=root,
                split=split,
                pre_transform=lambda g: transform_wrapper(
                    g, fn_final_transform
                ),
            )
        case "ZINC12k" | "ZINC12kNoE":
            from torch_geometric.datasets import ZINC

            fn_dataset = lambda split: ZINC(
                root=root,
                subset=True,
                split=split,
        ...
        case "ogbg_molhiv":
            ...
            fn_dataset = lambda split: ogbg_dataset_wrapper(
                "ogbg_molhiv",
                root=root,
                split=split,
                pre_transform=pre_transform,
    ...
    match name:
        case "P3R":
            ...
            fn_dataset = lambda: P3RDataset(
                root=root, pre_transform=pre_transform
            )
        case "EXP":
            from datasets.pickle import EXPDataset

            fn_dataset = lambda: EXPDataset(
                root=root, pre_transform=pre_transform
            )
